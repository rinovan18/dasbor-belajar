...
function getStudentRoster(kelasTarget) {
  ...
  const statusAktivitas = (total) => {
    ...
    if (total >= 3) return { statusAktivitas: "Kurang Konsisten", emoji: "⚠️" };
...
function verify(p) {
  ...
  const nis = _teks(p.nis || "");
  const u = users.rows.find((r) => {
    const s = _teks(_val(r, users.header, "StudentID"));
    return sid ? s === sid : _teks(_val(r, users.header, "NIS")) === nis;
  });

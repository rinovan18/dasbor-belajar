...
export class TimerKuis extends I18NMixin(DDDSuper(LitElement)) {
    ...
    return [
      ...
      css`
        ...
        button {
          ...
          border-radius: var(--ddd-radius-md); border: 1px solid var(--ddd-theme-polaris-border);
  ...
  render() {
    ...
    return html`
      <div class="timer-card" role="timer" aria-live="polite" aria-label="${this.t.title}: ${this._format(this._remaining)}">
        <div class="meta">
          <span class="title">${this.t.title}</span>
          <span class="time ${low ? "warn" : ""}" ${low ? 'aria-label="Waktu hampir habis: ' + this._format(this._remaining) + '"' : ''}>${this._format(this._remaining)}</span>
        </div>
        <div class="controls">
          ${this.hideControls
            ? ""

...
export class QuestionRenderer {
   ...
   return html`
     <div class="choices-container">
       ${choices.map((choice, i) => {
         ...
         let cls = "choice-row";
         if (isSelected && answered) cls += isCorrect ? " correct" : " wrong";
         ...
         return html`
           ...
           >${isSelected ? "✓ " : ""}${huruf[i] || i + 1}. ${choice}
           ${pilImg ? html`<br /><img class="choice-image" src="${pilImg}" alt="Gambar pilihan ${huruf[i] || i + 1}" loading="lazy" />` : ""}
           </button>
         `;
       })}
     </div>
     ${feedbackText ? html`<div class="feedback ${feedbackPositive ? 'correct' : 'wrong'}">${feedbackText}</div>` : ''}
  ...
  static renderPGKompleks(soal, state) {
    const { selectedAnswers, answered, feedbackText, feedbackPositive } = state;
    ...
    return html`
      <div class="choices-container">
        ${soal.choices.map((choice, i) => {
          ...
          return html`
            <div class="${cls}" @click=${() => state.onToggle(i)}>
  ...
  static renderPGK(soal, state) {
    const { pgkAnswers, answered, feedbackText, feedbackPositive } = state;
    return html`
  ...
  static renderMatching(soal, state) {
    const { matchAnswers, answered, feedbackText, feedbackPositive } = state;
    return html`
          ...
          ${(soal.rightItems || []).map((item, i) => {
            ...
            return html`
              <div class="match-item right ${selectedLeft !== null ? 'matched' : ''}" 
                ...
                ${selectedLeft !== null ? html`<span class="match-connector">← ${soal.leftItems[selectedLeft]}</span>` : ''}
              </div>
            `;
          })}
        </div>
  ...
  static renderShortAnswer(soal, state) {
    const { shortAnswerText, answered, feedbackText, feedbackPositive } = state;
    return html`

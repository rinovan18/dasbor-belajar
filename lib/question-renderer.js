...
export class QuestionRenderer {
  ...
  static renderMC(soal, state) {
    const { selected, selectedAnswers, answered, editable, feedbackText, feedbackPositive, hideAnswers } = state;
    ...
    return html`
      <div class="choices-container">
        ${choices.map((choice, i) => {
          ...
          const disabled = answered && !editable;
      ...
      ${isMulti && (!answered || editable) ? html`<button class="btn-submit" @click=${state.onSubmit}>Kirim Jawaban (${selectedAnswers.size} dipilih)</button>` : ''}
    `;
  }
  ...
  static renderPGKompleks(soal, state) {
    const { selectedAnswers, answered, editable, feedbackText, feedbackPositive, hideAnswers } = state;
  ...
  static renderPGK(soal, state) {
    const { pgkAnswers, answered, editable, feedbackText, feedbackPositive, hideAnswers } = state;
    return html`
  ...
  static renderMatching(soal, state) {
    const { matchAnswers, answered, editable, feedbackText, feedbackPositive, hideAnswers } = state;
    return html`
...
${!answered || editable ? html`<button class="btn-submit" @click=${state.onSubmit}>Kirim Jawaban</button>` : ''}
  ...
  static renderShortAnswer(soal, state) {
    const { shortAnswerText, answered, editable, feedbackText, feedbackPositive, hideAnswers } = state;
    return html`
